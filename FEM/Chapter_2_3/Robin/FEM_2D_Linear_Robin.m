...
function [uh,err] = FEM_2D_Linear_Robin(input_h,fig_flag,err_flag,err_types)
%% 输入网格信息
left = -1;
right = 1;
down = -1;
up = 1;
boundary_type = [-3 -1 -1 -1];% 边界条件 -1 -> 'D', -2 -> 'Neu', -3 -> 'Robin'
...
A = A1 + A2;
%% 使用边界条件信息矩阵调整边界条件
[A,b] = boundary_adjust(A,b,P,T,Pb,Tb,B_f_edges,B_m_edges,Nb,Nb,@func_boundary,@func_c,@func_p,@func_q,@func_r,dim,type);% 调用调整器

%% 求解并进行误差分析
uh = A \ b;
err = [];
if fig_flag == true
    show_eval(@func_real_u, uh, Pb, f_X, f_Y, "Linear 2D FEM")% 调用可视化
end
if err_flag == true
