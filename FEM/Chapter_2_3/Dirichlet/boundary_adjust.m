% 边界条件调整器
function [A,b] = boundary_adjust(A,b,P,T,Pb,Tb,B_f_edges,B_m_edges,Nb_test,Nb_trial,func_boundary,func_c,func_p,func_q,func_r,dim,type)
...
[A,b] = check_dirich(A,b,Pb,B_f_edges,func_boundary);
end

function [A,b] = check_dirich(A,b,Pb,B_f_edges,func_boundary)
idx = (B_f_edges(1,:) == -1);
row = B_f_edges(2,idx);
A(row,:) = 0;
diag_A = diag(A);% 提取对角线
...
b(row) = func_boundary(Pb(1,row),Pb(2,row));
end
