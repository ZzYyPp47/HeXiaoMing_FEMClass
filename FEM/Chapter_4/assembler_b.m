...
function b = assembler_b(int_type, Nf_test, Nm, P, T, Nb_test, basis_test, Tb_test, func_f, test_dx, test_dy, t0)
...
parfor ii = 1:Nm% 跑遍每个单元
    ...
    for beta = 1:1:Nb_test
        S(beta,1) = get_local_integral_b(int_type, beta, basis_test, func_f, test_dx, test_dy, vertices, t0);
    end
    temp{ii} = S;% 将单元负载矩阵存入元胞中
end
for ii = 1:1:Nm
  b(Tb_test(:, ii),1) = b(Tb_test(:, ii),1) + temp{ii};
end
end