...
t_s = tic;
for ii = 1:N
    [~,result_u(ii,:),result_p(ii,:)] = FEM_2D_Taylor_Hood_Dirichlet([1/2^(ii + 2),1/2^(ii + 2)],false,true,["max","L^inf","L^2","H^1(semi)"]);
    time = toc(t_s);
    fprintf('Completed task num: %d, time passed: %.8f s, Total: %d\n',ii,time,N);
end
% u
result_order_u = log(result_u(1:end - 1,3:end)./result_u(2:end,3:end)) ./ log(result_u(1:end - 1,1)./result_u(2:end,1));
...
result_u = table(sym(result_u(:,1)),sym(result_u(:,2)),result_u(:,3),result_u(:,4),result_u(:,5),result_u(:,6),...
...
disp(result_order_u);
% p
result_order_p = log(result_p(1:end - 1,3:end)./result_p(2:end,3:end)) ./ log(result_p(1:end - 1,1)./result_p(2:end,1));
...
result_p = table(sym(result_p(:,1)),sym(result_p(:,2)),result_p(:,3),result_p(:,4),result_p(:,5),result_p(:,6),...
               'VariableNames',{'hx','hy','Max Absolute Error','L^infty Error','L^2 Error','H^1(semi) Error'});
fprintf("p: \n");
disp(result_p);
fprintf("Convergence Order: \n");
