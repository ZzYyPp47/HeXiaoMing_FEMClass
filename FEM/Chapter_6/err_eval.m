...
% h = [hx hy]
function err = err_eval(real_u, left, right, down, up, uh, Nm, Nb, P, T, Pb, Tb, dim, type, h, err_types)
err = h;
...
for ii = 1:1:length(err_types)
    switch err_types(ii)
        case "max"% 节点max误差
            temp = max(abs(real_u(Pb(1,:),Pb(2,:),0,0)' - uh));
        ...
        case "H^1"% H^1误差
            ...
            temp = sqrt(temp);
        case "all"
            temp = err_eval(real_u, left, right, down, up, uh, Nm, Nb, P, T, Pb, Tb, dim, type, h, ["max","L^inf","L^2","H^1","H^1(semi)"]);
