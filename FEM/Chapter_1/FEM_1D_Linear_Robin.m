...
function [uh,err] = FEM_1D_Linear_Robin(input_h,fig_flag,err_flag,err_types)
%% 输入网格信息
left = 0;
right = 1;
hx = input_h(1);% x 网格步长
Nx = (right - left) / hx;% x网格单元数
b_type = [-3 -1];% 边界条件 -1 -> 'D', -2 -> 'Neu', -3 -> 'Robin'
% Neu/Robin = u'+qu=p
extra_info = [-1 1];% 为 Neu/Robin 准备的额外信息
extra_info_q = [1 0];% 为 Neu/Robin 准备的额外信息(q)
extra_info_p = [1 0];% 为 Neu/Robin 准备的额外信息(p)
...
err = [];
if fig_flag == true
    show_eval(@func_real_u, uh, Pb, "Linear 1D FEM")% 调用可视化
end
if err_flag == true
