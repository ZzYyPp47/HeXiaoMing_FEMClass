% 边界条件调整器
function [A,b] = boundary_adjust(A,b,P,T,Pb_trial,Tb,B_f_edges,B_m_edges,Nf,Nb_test,Nb_trial,func_boundary1,func_boundary2,func_c,func_p,func_q,func_r,dim,type)
...
[A,b] = check_dirich(A,b,Nf,Pb_trial,B_f_edges,func_boundary1,func_boundary2);
end

function [A,b] = check_dirich(A,b,Nf,Pb_trial,B_f_edges,func_boundary1,func_boundary2)
idx = (B_f_edges(1,:) == -1);
row = B_f_edges(2,idx);
% eqa 1
A(row,:) = 0;
...
b(row) = func_boundary1(Pb_trial(1,row),Pb_trial(2,row));
% eqa 2
A(row + Nf,:) = 0;
diag_A = diag(A);% 提取对角线
diag_A(row + Nf) = 1; % 将对应对角元置换为1
A = spdiags(diag_A,0,A);% 替换主对角元
b(row + Nf) = func_boundary2(Pb_trial(1,row),Pb_trial(2,row));
end
